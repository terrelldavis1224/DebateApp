    ...
    var body: some View {
        ...
        if userId == "" {
            ...
            if currentViewShowing == "signup" {
                ...
                QuizView(currentViewShowing: $currentViewShowing)
            }
            
            // if we came from login, go to homepage
            if currentViewShowing == "login" {
                let _ = print("Entering Login")
                HomeView(currentViewShowing: $currentViewShowing)
                //ProfileView(currentViewShowing: $currentViewShowing)
            }
            
            if currentViewShowing == "quiz" {
                let _ = print("Entering quiz")
                HomeView(currentViewShowing: $currentViewShowing)
            }
        }
    }
    ...
    func signup() {
        Auth.auth().createUser(withEmail: User.email, password: User.password) {
            ...
            if let result = result {
                ...
                if let username = User.email.split(separator: "@").first {
                    updateDisplayName(newDisplayName: String(username))
                } else {
